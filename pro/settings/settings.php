...
function settings_validate( $input ) {
    error_log('Starting settings validation'); // Log start
    ...
    if ( isset( $input['default_status_term'] ) && in_array( $input['default_status_term'], $status_slugs ) ) {
    ...
    if ( isset( $input['restricted_courses'] ) && is_array( $input['restricted_courses'] ) ) {
        ...
        foreach ( $input['restricted_courses'] as $course_id ) {
            // Ensure the course ID is numeric
    ...
    $validated_settings['custom_idea_heading'] = ! empty( $input['custom_idea_heading'] ) ? sanitize_text_field( $input['custom_idea_heading'] ) : '';
    ...
    $validated_settings['custom_display_ideas_heading'] = ! empty( $input['custom_display_ideas_heading'] ) ? sanitize_text_field( $input['custom_display_ideas_heading'] ) : '';

    // Validate 'hide_from_rest'
    $validated_settings['hide_from_rest'] = ! empty( $input['hide_from_rest'] ) && $input['hide_from_rest'] == '1' ? 1 : 0;

    error_log('Completed settings validation'); // Log end
