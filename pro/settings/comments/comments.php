...
function wp_roadmap_pro_enable_comments() {
    $options = get_option('wp_roadmap_settings', []); // Retrieve the current settings
    $allow_comments = isset($options['allow_comments']) ? $options['allow_comments'] : '';
    echo '<input type="checkbox" name="wp_roadmap_settings[allow_comments]" value="1"' . checked(1, $allow_comments, false) . '/>';
}
