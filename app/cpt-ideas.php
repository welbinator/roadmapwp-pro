...
function auto_assign_new_idea_status( $post_id, $post, $update ) {
	...
	$filter_term = term_exists( 'New Idea', 'status' );

	// If the term doesn't exist, add it.
	if ( $filter_term === 0 || $filter_term === null ) {
		$filter_term = wp_insert_term( 'New Idea', 'status' );
	}

	// Check for errors.
	if ( is_wp_error( $filter_term ) ) {
		error_log( 'Error auto-assigning "New Idea" status: ' . $filter_term->get_error_message() );
		return;
	}
