...
function new_idea_form_shortcode() {
	...
	if ( isset( $_GET['new_idea_submitted'], $_GET['nonce'] )
		&& '1' === sanitize_text_field( wp_unslash( $_GET['new_idea_submitted'] ) )
		&& wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['nonce'] ) ), 'new_idea_submitted' )
	) {
		...
		<div class="new_idea_form__frontend">
			...
			<form action="<?php echo esc_url( $form_action ); ?>" method="post">
				<ul class="rmwp__flex-outer">
					...
					<?php
					$taxonomies = get_object_taxonomies( 'idea', 'objects' ); // Get all taxonomies for the 'idea' post type.
...
function handle_new_idea_submission() {
	...
	if ( isset( $_SERVER['REQUEST_METHOD'] )
		...
		&& isset( $_POST['idea_title'], $_POST['wp_roadmap_new_idea_nonce'] )
	) {
	...
	if ( isset( $_SERVER['REQUEST_METHOD'] )
		&& 'POST' === sanitize_text_field( wp_unslash( $_SERVER['REQUEST_METHOD'] ) )
		&& ! empty( $submission_nonce )
		&& $idea_id
	) {
		if ( ! empty( $_POST['idea_taxonomies'] ) && is_array( $_POST['idea_taxonomies'] ) ) {
			$sanitized_taxonomies = map_deep( wp_unslash( $_POST['idea_taxonomies'] ), 'sanitize_text_field' );
			foreach ( $sanitized_taxonomies as $tax_slug => $term_ids ) {
				$tax_slug = sanitize_key( $tax_slug );
				if ( ! taxonomy_exists( $tax_slug ) ) {
					continue; // Skip processing for non-existing taxonomies.
				}
				$term_ids = array_map( 'absint', (array) $term_ids ); // Ensure term IDs are positive integers.
