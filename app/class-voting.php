...
class VotingHandler {
	...
	public static function render_vote_button( $idea_id, $vote_count ) {
		...
		$idea_id_attr = intval( $idea_id );
		?>
	<div class="flex items-center idea-vote-box" data-idea-id="<?php echo esc_attr( (string) $idea_id_attr ); ?>">
			<button class="inline-flex items-center justify-center text-sm font-medium h-10 bg-blue-500 text-white px-4 py-2 rounded-lg idea-vote-button">
				...
				<div class="idea-vote-count"><?php echo esc_html( (string) $vote_count ); ?></div>
			</button>
		</div>
		<?php
	}
